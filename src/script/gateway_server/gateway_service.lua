...
function send_to_server(client_session,raw_data)
   ...
   if cmd == cmd_module.LoginLogicReq then
		 ...
		 --获取client连接gateway的的真实公网ip,用户udp的转发
		 ...
		 --设置客户端的tcp_ip到结构体,这里的ip客户端默认传输的一个本地127.0.0.1地址
		 --以为client也无法知道自己的真实地址，只有在连接gateway后，在gateway端获取
		 --然后转发到logic服务,这样logic服务就知道client的公网ip了
		 dbody.udp_ip = tcp_ip
