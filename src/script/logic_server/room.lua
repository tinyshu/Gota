...
function room:game_start()
	...
	--等待3s后，开始帧同步，20fps,每次间隔就是60ms(启动一个间隔50ms定时执行的定时器)
	--注意这里create_timer需要绑定一个匿名函数，在匿名函数里面在调用类成员函数 
	self.frame_time = timer_wrapper.create_timer(function()
           self:do_logic_frame_sync()
		   end,-1,5000,LOGIC_FRAME)
...
function room:push_next_frame(next_frame_opts)
	...
	if p.cur_sync_frame_id <  next_frame_opts.frameid - 1 then
	   ...
	   --print("play p.cur_sync_frame_id:"..p.cur_sync_frame_id)
	end

	--判断当前传入的帧是否是和服务器代收的frameid匹配
    if  next_frame_opts.frameid ~= self.frameid then
	     --print("next_frame_opts.frameid ~= self.frameid next_frame_opts.frameid:"..next_frame_opts.frameid.." self.frameid"..self.frameid)
		return
	end
