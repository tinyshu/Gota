...
static void on_connection(uv_stream_t* server, int status) {
	...
	/*
	uv_tcp_t new_client是libuv层的tcp链接对象
	session s 是框架保存客户端回话对象,是框架表示一个客户端链接,上层数据发送都是调用该对象接口
	io_package io_data 当前接受到的数据一个缓存区
	*/
	...
	//on_read_alloc_buff 有数据进来，用于分配数据读取缓存区在该函数实现
	//on_after_read 读取到数据后被调用
...
void start_server(char* ip, int port) {
	...
	//设置一个被动套接字，在有新连接，回调on_connection函数
